...
export default function LoginPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if(email === 'admin@qalbie.id') {
      // Mock login as admin
      login({ name: 'Admin', tier: 'premium', role: 'admin' }, 'dummy-jwt');
      navigate('/admin');
    } else {
      // Mock login
      login({ name: email.split('@')[0], tier: 'premium' }, 'dummy-jwt-token');
      navigate('/dashboard');
    }
  };
    ...
    <div className="flex flex-col min-h-screen bg-neutral-50 font-sans">
      ...
      <main className="flex-1 max-w-md mx-auto w-full px-6 flex flex-col justify-center pb-20">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <h1 className="text-3xl font-display font-bold text-neutral-900 mb-2">Selamat Datang Kembali</h1>
          <p className="text-neutral-500 mb-8">Masuk untuk melanjutkan perjalanan ketenanganmu bersama Qalbie.</p>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div>
              ...
              <div className="text-right mt-2">
                <Link to="#" className="text-sm font-semibold text-primary-600 hover:underline">Lupa kata sandi?</Link>
              </div>
            </div>

            <button type="submit" className="w-full bg-primary-600 text-white font-bold py-3.5 rounded-full mt-4 hover:bg-primary-700 transition-colors shadow-lg shadow-primary-600/20">
              Masuk
            </button>
          </form>
          ...
          <div className="mt-8 text-center">
            <p className="text-sm text-neutral-600">
              Belum punya akun? <Link to="/register" className="font-bold text-primary-600 hover:underline">Daftar sekarang</Link>
            </p>
          </div>
        </motion.div>
      </main>
